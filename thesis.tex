...
I'd also like to thank Alexi Turcotte for showing me that with a good friend and enough coffee anything is possible. % I LOVE CAMELS
...
%CRA implementation in C, using the MPZ library: \hfill \ref{theC}\\

\newpage 
\singlespacing
\lstinputlisting[label=cra, caption={CRA reconstruction for integers}, basicstyle=\footnotesize\ttfamily]{test/cra_int.mpl}
\doublespacing

\singlespacing
\lstinputlisting[label=frec, caption={Rational reconstruction wrapper for polynomials over the integers}, basicstyle=\footnotesize\ttfamily]{test/intpolyrecon.mpl}
\doublespacing

\singlespacing
\lstinputlisting[label=bcra, caption={Groebner basis computation via the CRA}, basicstyle=\footnotesize\ttfamily]{test/Basis_CRA.mpl}
\doublespacing

\singlespacing
\lstinputlisting[label=bh, caption={Groebner basis computation via Hensel, including helper methods}, basicstyle=\footnotesize\ttfamily]{test/Basis_Hensel.mpl}
\doublespacing

\singlespacing
\lstinputlisting[label=timer, caption={Timer to run specified problem with specified method}, basicstyle=\footnotesize\ttfamily]{test/timer.mpl}
\doublespacing

\singlespacing
\lstinputlisting[label=bash, caption={Batch runner to run all 25 selected problems with all methods/algorithms}, basicstyle=\footnotesize\ttfamily]{test/theTest/runner.sh}
\doublespacing

% \singlespacing
% \lstinputlisting[label=theC, caption={CRA implementation in C, using the MPZ library}, basicstyle=\footnotesize\ttfamily]{test/pls.c}
% \doublespacing
