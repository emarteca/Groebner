...
-- Trends: FGB is faster (wow); BB > F4 > FGB for time pretty consistently\\
-- graphs here? hmmmmmm\\
-- Hensel is shit; CRA is ok sometimes, Maple is always faster\\
-- give up on Hensel - it's doomed anyway, since it literally needs BB to run which is trash compared to everything else\\
-- hopeful: if the CRA recon is slow (or even the Farey), we could maybe optimize using external C calls (explained somewhere - maybe appendix about external C calls and how they work?)\\
-- so, let's profile the CRA to see what proportion is what\\
-- the sections are, modular basis comp through wte basis alg, cra recon, and farey recon\\
-- using the built-in farey recon, but my own works, it's just a bit slower (could maybe optimize in C), or CRA in C\\
-- results below, per alg\\
...
-- that awkward moment when the mod basis comp is actually taking up all of the time\\
-- graphs of maple vs only mod basis time\\
-- so there's literally nothing we can do :'(\\
-- except rewrite the Groebner package\\
-- classic\\

-- so yeah, then graph per image and it's fine\\
-- also explain why this is literally impossible b/c limits and wordsize kappa\\


-- future work\\
-- sympy/mathematica/etc (check what they have)\\
-- pls to rewrite maple groebner\\
