...
# Groebner basis via Hensel reconstruction
# takes in a basis B, monomial ordering ord, and initial prime thePrime
Basis_Hensel := proc( B,  ord,  thePrime)
  local Gp,  Z1,  Znorm,  Gpnorm,  curG,  curZ,  i,  oldRec,  curRec,  
  timings,  theTime,  isGoodPrime;

  
  # can only use buchberger, as the Z1 matrix is required
  Gp,  Z1 := Basis( B,  ord,  characteristic=thePrime,  method=buchberger,  output=extended):

  Gp := Gp * lcm( op( denom( Gp))); # normalize won't crash with fractions 
                                    # (i.e. convert to ints)
  ...
  while oldRec <> curRec or curRec = FAIL do
    ...
    curRec,  isGoodPrime := basisrecon( thePrime^i,  curG);
  ...
  # rational reconstruction using maple iratrecon
  return iratrecon( curG,  thePrime^(i-1)),  verifyBasis( B,  ord,  curG,  thePrime,  i),  timings;

end:

     
# now follow some helper methods for hensel
